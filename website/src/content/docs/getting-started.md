...
:::note
Image in a **private** repo? Nothing to set up on the server. The agent has no GitHub identity of
its own, so the Action **forwards the run's short-lived token** (the same `GITHUB_TOKEN` it pushes
with) inside the signed deploy; the agent uses it only to read the image's cosign signature and pull
it, in memory, then discards it — nothing is stored on the server. Just keep `packages: write` in the
workflow `permissions` (the scaffold already has it). The image and code stay private.
:::
...
Run it with **sudo** on the server: the config (`0600` root), the secret files and the service status
all need root. Without sudo it still checks everything it can and tells you to re-run
